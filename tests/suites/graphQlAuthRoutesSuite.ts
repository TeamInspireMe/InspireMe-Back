...
const graphQlAuthRoutesSuite = (server: supertest.SuperTest<supertest.Test>) =>
	describe('GraphQL - Auth routes', () => {
        ...
        signUp(username: "bobi22", email: "bob@gmail.com", password: "boob1")
        {uuid, username, email, password}
			...
			it('Sign Up with an already used username should return 200 AND body should contains errors', async done => {
				const query = `mutation {
        signUp(username: "bobi22", email: "bob@gmail.com", password: "boob1")
        {uuid, username, email, password}
      }`;
