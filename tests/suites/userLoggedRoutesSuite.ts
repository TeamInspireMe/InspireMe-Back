...
const userLoggedRoutesSuite = (server: supertest.SuperTest<supertest.Test>) =>
	describe('User routes', () => {
		...
		describe('UPDATE route', () => {
			it('Should return 200 if correct token - uuid - username - email', async done => {
				const res: request.Response = await server
					.put('/api/user/')
					.set('Accept', 'application/json')
					.set('Authorization', `Bearer ${permanentUserToken}`)
			...
			it('Should return 400 if correct token - uuid BUT empty username', async done => {
				const res: request.Response = await server
					.put('/api/user/')
					.set('Accept', 'application/json')
					.set('Authorization', `Bearer ${permanentUserToken}`)
			...
			it('Should return 400 if correct token - uuid BUT incorrect username AND email', async done => {
				const res: request.Response = await server
					.put('/api/user/')
					.set('Accept', 'application/json')
					.set('Authorization', `Bearer ${permanentUserToken}`)
			...
			it('Should return 400 if correct token - uuid BUT incomplete data (username or email)', async done => {
				const resWithNousername: request.Response = await server
					.put('/api/user/')
					.set('Accept', 'application/json')
					.set('Authorization', `Bearer ${permanentUserToken}`)
					.send({
				...
				expect(resWithNousername.status).toBe(400);
				expect(resWithNoEmail.status).toBe(400);
				done();
			});

			it('Should return 400 if correct token BUT no UUID and incomplete data (username or email)', async done => {
				const resWithNousername: request.Response = await server
					.put('/api/user/')
					.set('Accept', 'application/json')
					.set('Authorization', `Bearer ${permanentUserToken}`)
			...
			it('Should return 500 if correct token BUT no UUID', async done => {
				const res: request.Response = await server
					...
					.send({
						// no ID
						username: 'newusername', // incorrect
